...
must_haves:
  truths:
    ...
    - "Notification center dropdown displays last 50 notifications with mark-as-read, navigate, and retry actions for failed jobs"
  ...
  key_links:
    ...
    - from: "src/components/notifications/notification-center.tsx"
      to: "src/app/api/jobs/[jobId]/retry/route.ts"
      via: "retry button POSTs to job retry endpoint for job:failed notifications"
...
<task type="auto">
  ...
  <verify>
    <automated>cd /Users/patrickbaumfalk/Projekte/AI-Lawyer && npx tsc --noEmit --pretty 2>&1 | head -30 && npx tsx scripts/build-server.ts 2>&1 && ls -la dist-server/index.js && echo "Server bundle OK"</automated>
...
<task type="auto">
  ...
  <action>
    ...
    6. Create `src/components/notifications/notification-center.tsx` — Dropdown notification list:
       ...
       - **Retry button for failed jobs:** For notifications with type "job:failed", render an "Erneut versuchen" action button.
         - On click: POST to `/api/jobs/${notification.data.jobId}/retry` endpoint
         - Show spinner/disabled state while retrying
         - On success: show toast "Job wird erneut verarbeitet" and dismiss the failure notification
         - On error: show toast "Wiederholung fehlgeschlagen"
         - Per locked decision: user sees "PDF-Verarbeitung fehlgeschlagen. Erneut versuchen?" with retry capability
  ...
  <done>
    ...
    - Notification center shows last 50 notifications with title, message, time, read/dismiss actions, and retry button for job:failed notifications
