...
must_haves:
  truths:
    ...
    - "seedAmtlicheFormulare() inserts 6 amtliche Formulare with {{PLATZHALTER}} into muster_chunks on first worker boot — idempotent via SystemSetting version key — no MinIO fetch required"
    - "insertMusterChunks() chunks + embeds a Muster file and writes to muster_chunks with kanzleiEigen propagated from parent Muster; when optional content string is provided, extractMusterFullText is skipped entirely"
  artifacts:
    ...
    - path: "src/lib/muster/ingestion.ts"
      provides: "extractMusterFullText, insertMusterChunks (optional content param), searchMusterChunks"
  ...
  key_links:
    ...
    - from: "src/lib/muster/seed-amtliche.ts:seedAmtlicheFormulare"
      to: "src/lib/muster/ingestion.ts:insertMusterChunks"
      via: "passes amtliches.content string — no MinIO fetch"
      pattern: "insertMusterChunks.*content"
---
...
2. Creates a synthetic system-user Muster row per amtliches Muster via `prisma.muster.create` with `isKanzleiEigen: false`, `nerStatus: "INDEXED"`, `minioKey: "seed/placeholder"`, `mimeType: "text/plain"`, `uploadedById` = first ADMIN user in DB
3. Calls `insertMusterChunks(muster.id, amtliches.content)` from ingestion.ts for each — passing the hardcoded content string directly so no MinIO fetch occurs
...
`insertMusterChunks(musterId: string, content?: string): Promise<{ inserted: number }>`

IMPORTANT: The `content` parameter is the key to avoiding MinIO errors for seeded amtliche forms.
- Loads Muster from DB with `select: { minioKey, mimeType, isKanzleiEigen }`
- Text extraction path (choose based on `content` param):
  - If `content` is provided (non-empty string): use it directly as `text` — skip `extractMusterFullText` entirely. This is the path used by `seedAmtlicheFormulare()` to avoid a MinIO fetch for hardcoded content.
  - If `content` is NOT provided (or empty): call `extractMusterFullText(muster.minioKey, muster.mimeType)` to fetch from MinIO. This is the path used by `processMusterIngestionJob()` for real uploaded files.
...
Use `import pdfParse from "pdf-parse"` (already in package.json as ^2.4.5).
  ...
  <verify>
    <automated>cd /Users/patrickbaumfalk/Projekte/AI-Lawyer && npx tsc --noEmit 2>&1 | head -20 && grep -l "seedAmtlicheFormulare\|insertMusterChunks\|searchMusterChunks\|MusterChunkResult" src/lib/muster/ingestion.ts src/lib/muster/seed-amtliche.ts && grep "content\?" src/lib/muster/ingestion.ts</automated>
  </verify>
  <done>Both files exist; `npx tsc --noEmit` exits 0; ingestion.ts exports MusterChunkResult, extractMusterFullText, insertMusterChunks (with optional content?: string param), searchMusterChunks; seed-amtliche.ts exports AMTLICHE_MUSTER (6 entries) and seedAmtlicheFormulare; seedAmtlicheFormulare calls insertMusterChunks(muster.id, amtliches.content) — no MinIO fetch for amtliche Formulare</done>
</task>
...
- `grep "content\?" src/lib/muster/ingestion.ts` — optional content param present in insertMusterChunks signature
- `grep "insertMusterChunks.*content" src/lib/muster/seed-amtliche.ts` — seed calls insertMusterChunks with content argument
</verification>
...
- src/lib/muster/ingestion.ts exports MusterChunkResult interface, extractMusterFullText, insertMusterChunks(musterId, content?) with optional content param, searchMusterChunks with 1.3x kanzlei boost
- When content param is provided, insertMusterChunks does NOT call extractMusterFullText or getFileStream — MinIO is not contacted
...
- seedAmtlicheFormulare passes amtliches.content to insertMusterChunks — no MinIO fetch for seeded forms
