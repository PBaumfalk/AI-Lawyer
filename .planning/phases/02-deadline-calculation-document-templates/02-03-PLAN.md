...
<objective>
Rebuild the OnlyOffice integration for stable save/load, co-editing, Track Changes, Comments, and versioning by fixing the document.key strategy and enhancing the callback handler. Add DOCX-to-PDF conversion endpoint.

Purpose: The current implementation uses `Date.now()` in document.key, breaking co-editing (each user gets a separate session). This rebuild ensures reliable document editing as the foundation for the template system, Briefkopf application, and PDF export in Plan 02-04.

Output: Enhanced onlyoffice.ts library, fixed callback handler with version management, conversion API endpoint, and enhanced editor component.
</objective>
...
<task type="auto">
  <name>Task 1: Stable Document Key + Enhanced Callback Handler + Versioning Schema</name>
  <files>
    prisma/schema.prisma,
    src/lib/onlyoffice.ts,
    src/app/api/onlyoffice/callback/route.ts,
  ...
  <action>
    ...
    **2. Stable Key Generation (src/lib/onlyoffice.ts):**
    ...
    - Accept `version: number` as required parameter (no more Date.now()).
    ...
    **4. Enhanced Config Endpoint (src/app/api/onlyoffice/config/[dokumentId]/route.ts):**
    - Load Dokument from DB including `version`.
    - Call `buildEditorConfig({ ..., version: dokument.version })`.
  ...
  <verify>
    <automated>npx prisma validate && npx tsc --noEmit src/lib/onlyoffice.ts src/app/api/onlyoffice/callback/route.ts</automated>
    <manual>Open a document in OnlyOffice, verify stable key in editor config, make edits, close, reopen and verify content persisted with new version</manual>
  </verify>
  <done>Document key format is {dokumentId}_v{version}. Callback status 2 increments version and creates DokumentVersion snapshot. Status 6 saves without incrementing. Track Changes and Comments enabled in editor config. Co-editing works (same key for same document+version).</done>
</task>
...
<task type="auto">
  ...
  <verify>
    ...
    <manual>Open document in OnlyOffice, use Track Changes, add Comment, export as PDF, view version history, create named snapshot, restore old version</manual>
  </verify>
  <done>Conversion API converts DOCX to PDF via OnlyOffice. Version history API provides list/create/restore. Editor component shows version panel, Track Changes controls, connection status, and PDF export button. Restore creates new version (old preserved).</done>
</task>
