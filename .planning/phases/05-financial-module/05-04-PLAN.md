...
<task type="auto">
  <name>Task 1: E-Rechnung Library + Integration + Tests</name>
  <files>
    src/lib/finance/invoice/e-rechnung.ts
    src/lib/finance/invoice/__tests__/e-rechnung.test.ts
    src/app/api/finanzen/rechnungen/[id]/e-rechnung/route.ts
  ...
  <action>
    ...
    **1b. XRechnung Structural Tests** (src/lib/finance/invoice/__tests__/e-rechnung.test.ts):
    Create vitest test file covering:
    - `generateXRechnungXml()` output contains correct CII namespace: `urn:un:unece:uncefact:data:standard:CrossIndustryInvoice:100`
    - XML contains ExchangedDocument with TypeCode=380 (commercial invoice)
    - XML contains BT-1 (invoice number), BT-2 (issue date), BT-5 (currency EUR)
    - XML contains seller (BG-4) and buyer (BG-7) blocks
    - XML contains at least one invoice line (BG-25)
    - XML contains VAT breakdown (BG-23) with correct rate
    - `mapInvoiceToEN16931()` maps all mandatory BT fields
    - At least 8 test cases for XML structure validation
  ...
  <verify>
    <automated>npx vitest run src/lib/finance/invoice/__tests__/e-rechnung.test.ts && npx tsc --noEmit</automated>
