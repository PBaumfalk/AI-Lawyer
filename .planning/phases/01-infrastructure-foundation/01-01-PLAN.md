...
<task type="auto">
  ...
  <action>
    ...
    9. Update `.env.example` with new env vars (documented with description, default, required/optional):
       ...
       - ONLYOFFICE_INTERNAL_URL (optional, default http://localhost:8080, description: Internal URL for OnlyOffice health checks — used by /api/health endpoint; in Docker use http://onlyoffice:80)
  </action>
  <verify>
...
<task type="auto">
  ...
  <action>
    ...
    4. Update `docker-compose.yml`:
       ...
       - Add healthcheck to `app` service (per locked decision "Docker Compose health checks use /api/health"):
         ```yaml
         healthcheck:
           ...
           start_period: 60s
         ```
       - Update `worker` depends_on to also include `app: condition: service_healthy` (worker needs app's health endpoint available and Redis connected before processing jobs)
  ...
  <done>
    ...
    - App service has healthcheck directive using curl to /api/health (interval 30s, timeout 10s, retries 3, start_period 60s)
    - Worker depends_on includes app (service_healthy) in addition to redis and db
