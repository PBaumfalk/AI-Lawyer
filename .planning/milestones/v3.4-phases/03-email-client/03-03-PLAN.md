...
must_haves:
  truths:
    - "User can compose a NEW email with rich-text editing (bold, italic, links, lists, tables, images) — reply/forward handled inline in Plan 02"
    - "Compose popup opens as a floating popup for new emails that can be minimized/maximized, keeping inbox visible"
...
- `openCompose()` — add new blank compose instance (max 5 concurrent). No initialData — compose popup is for NEW emails only. Reply/forward is handled by inline reply in email-detail.tsx (Plan 02).
...
- Export `useComposeManager()` hook for triggering new compose from toolbar/sidebar compose button.
...
- NOTE: Reply/ReplyAll/Forward are NOT handled by the compose popup. Per locked user decision: "Reply/forward: opens inline in detail pane (compose area below original email)". The inline reply component is implemented in Plan 03-02 (email-inline-reply.tsx). The compose popup is ONLY for new email composition.
- Append signature to email body on open. Load via getSignatureForUser(). Insert at bottom of body.
