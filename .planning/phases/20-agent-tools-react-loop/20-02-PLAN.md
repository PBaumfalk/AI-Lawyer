...
5. Call `generateText`:
   ...
   const result = await generateText({
     ...
     experimental_repairToolCall: ollamaResponseGuard,  // From response-guard.ts (Plan 03) -- import conditionally or pass as param
...
6. **Stall handling:** If stall was detected during the loop:
...
8. **Error handling:** Wrap the entire generateText call in try-catch:
...
9. **Token tracking integration:** After successful run, call `wrapWithTracking` from `src/lib/ai/token-tracker.ts` for the aggregated usage.

NOTE on `experimental_repairToolCall`: This will be created in Plan 03 (response-guard.ts). For now, import it if it exists, or set to undefined. The orchestrator should accept an optional `repairToolCall` parameter in AgentRunOptions so Plan 03 can inject it.
  </action>
  <verify>
    <automated>cd /Users/patrickbaumfalk/Projekte/AI-Lawyer/.claude/worktrees/dynamic-sleeping-dream && npx tsc --noEmit --pretty 2>&1 | head -50</automated>
  </verify>
