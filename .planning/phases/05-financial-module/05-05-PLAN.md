...
<task type="auto">
  <name>Task 1: DATEV + SEPA Export Libraries + Bank Import</name>
  <files>
    src/lib/finance/export/types.ts
    src/lib/finance/export/datev.ts
    src/lib/finance/export/sepa.ts
  ...
  <action>
    ...
    **4. Bank CSV Parser** (src/lib/finance/banking/csv-parser.ts):
    ...
    **5. CAMT Parser** (src/lib/finance/banking/camt-parser.ts):
    ...
    **6. Duplicate Detection** (src/lib/finance/banking/dedup.ts):
    ...
    **7. Invoice Matcher** (src/lib/finance/banking/matcher.ts):
  ...
  <verify>
    ...
    <manual>Verify DATEV CSV has EXTF_ header, semicolons, German number format. Verify SEPA XML has correct namespace. Verify bank CSV parser handles Sparkasse format.</manual>
  </verify>
  <done>
    DATEV export produces valid Buchungsstapel CSV with EXTF_ header and SKR03/04 mapping. SEPA generates pain.001/008 XML. Bank CSV import detects format and deduplicates. Matcher suggests invoice matches. Match confirmation cascades to invoice status + Aktenkonto booking.
  </done>
</task>

<task type="auto">
  <name>Task 2: Time Tracking (Timer + Manual Entries + API)</name>
  <files>
    src/lib/finance/time-tracking/types.ts
    src/lib/finance/time-tracking/timer.ts
    src/app/api/finanzen/zeiterfassung/route.ts
  ...
  <verify>
    <automated>npx tsc --noEmit</automated>
    <manual>Verify timer start/stop/switch logic, Stundenhonorar Pflichtangabe enforcement, Taetigkeitskategorien defaults</manual>
  </verify>
  <done>
    Timer auto-starts per Akte, one active at a time, quick-switch. Stundenhonorar-Akten require Taetigkeitsbeschreibung on stop. Manual time entries with configurable categories. Time entries filterable and summable per Akte/User/period.
  </done>
</task>
