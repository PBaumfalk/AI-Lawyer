...
must_haves:
  truths:
    ...
    - "Token usage is tracked for all scan/briefing operations with monthly budget enforcement"
...
Note: Also wire email sync to trigger ai-scan. In the email sync processor (or IMAP connection manager's onNewEmail callback), after storing a new email, add job to 'ai-scan' queue with type='email'. Check existing email sync code for the right hook point.
  </action>
  <verify>
