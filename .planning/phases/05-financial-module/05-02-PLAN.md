...
must_haves:
  truths:
    - "Concurrent invoice creation never produces duplicate Rechnungsnummern (atomic UPSERT with RETURNING verified)"
...
<task type="auto">
  <name>Task 1: Prisma Schema Expansion + Invoice Backend Library + Tests</name>
  <files>
    prisma/schema.prisma
    src/lib/finance/invoice/types.ts
    src/lib/finance/invoice/nummernkreis.ts
  ...
  <action>
    ...
    **6. Invoice Tests** (src/lib/finance/invoice/__tests__/invoice.test.ts):
    Create vitest test file covering:
    - Nummernkreis: `getNextInvoiceNumber()` produces formatted numbers matching pattern (e.g., "RE-2025-0001")
    - Nummernkreis: sequential calls produce incrementing numbers (0001, 0002, 0003)
    - Status machine: ENTWURF -> GESTELLT is valid transition
    - Status machine: BEZAHLT -> ENTWURF is invalid (returns error)
    - Status machine: GESTELLT -> STORNIERT triggers Stornorechnung side effect
    - Status machine: all invalid transitions are rejected
    - At least 8 test cases for Nummernkreis + status flow
  </action>
  <verify>
    <automated>npx prisma generate && npx vitest run src/lib/finance/invoice/__tests__/invoice.test.ts && npx tsc --noEmit</automated>
