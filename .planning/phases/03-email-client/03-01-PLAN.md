...
must_haves:
  ...
  key_links:
    ...
    - from: "src/lib/email/imap/connection-manager.ts"
      ...
      via: "Socket.IO emit 'email:folder-update' event with { kontoId, ordnerId, ungeleseneAnzahl } consumed by FolderTree listener"
...
<task type="auto">
  <name>Task 2a: Create IMAP connection manager, sync engine, IDLE handler, folder sync, and email parser</name>
  <files>
    src/lib/email/imap/connection-manager.ts
    src/lib/email/imap/sync.ts
    src/lib/email/imap/idle-handler.ts
...
- `on("exists")` event: log new message count, call `syncNewMessages()` for incremental sync, emit Socket.IO event `email:folder-update` with { kontoId, ordnerId, ungeleseneAnzahl } to `mailbox:{kontoId}` room via `getSocketEmitter()`.
...
- `parseRawEmail(stream)` — wraps mailparser `simpleParser`, extracts headers, body (HTML + text), attachments metadata, returns `ParsedEmail` type.
  ...
  <verify>
    ...
    <manual>Check that all 5 IMAP files exist under src/lib/email/imap/ with correct exports</manual>
  </verify>
  <done>IMAP connection manager connects to mailboxes with IDLE, reconnects on failure with exponential backoff. Sync engine downloads emails from IMAP into EmailNachricht with attachments in MinIO. Folder sync mirrors IMAP folder structure. Parser extracts email data from raw MIME streams.</done>
</task>

<task type="auto">
  <name>Task 2b: Create SMTP transport, send processor, BullMQ queues, worker wiring, and LanguageTool docker service</name>
  <files>
    src/lib/email/smtp/transport-factory.ts
    src/lib/email/smtp/send-processor.ts
    src/lib/queue/queues.ts
...
- Add LanguageTool as optional service with profiles: ["full"]. Image: `erikvl87/languagetool:latest`, port 8010, with German language data volume. This is optional and won't block other functionality.
  ...
  <verify>
    <automated>cd /Users/patrickbaumfalk/Projekte/AI-Lawyer && npx tsc --noEmit --pretty 2>&1 | head -30</automated>
    <manual>Check that src/worker.ts imports and calls startImapConnections, registers emailSendWorker, and calls stopAllConnections on shutdown. Check emailSendQueue and emailSyncQueue appear in ALL_QUEUES.</manual>
  </verify>
  <done>SMTP send processor sends emails via BullMQ queue with 10-second delay support. All queues registered in ALL_QUEUES. Worker startup initializes IMAP connections, registers email send/sync workers, and graceful shutdown closes all connections. LanguageTool available as optional Docker service.</done>
</task>
