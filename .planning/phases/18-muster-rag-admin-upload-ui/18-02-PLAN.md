...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/lib/queue/processors/muster-ingestion.processor.ts"
      provides: "processMusterIngestionJob() — calls insertMusterChunks after INDEXED"
  ...
  key_links:
    ...
    - from: "src/lib/queue/processors/muster-ingestion.processor.ts"
      ...
      via: "after confirming nerStatus === INDEXED guard"
...
3. Call `insertMusterChunks(data.musterId)` from `@/lib/muster/ingestion`
