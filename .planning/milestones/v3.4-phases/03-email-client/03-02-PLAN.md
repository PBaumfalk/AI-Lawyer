...
must_haves:
  truths:
    ...
    - "Reply/forward opens inline below the original email body in the detail pane (not in compose popup)"
  ...
  artifacts:
    ...
    - path: "src/components/email/email-html-body.tsx"
      ...
      min_lines: 20
    - path: "src/components/email/email-inline-reply.tsx"
      provides: "Inline reply/forward compose area within email detail pane"
      min_lines: 80
  key_links:
    ...
    - from: "src/components/email/email-detail.tsx"
      ...
      pattern: "fetch.*api/emails/"
    - from: "src/components/email/email-detail.tsx"
      to: "src/components/email/email-inline-reply.tsx"
      via: "Reply/Forward buttons expand inline compose area below email body"
      pattern: "<EmailInlineReply"
---
...
<task type="auto">
  <name>Task 2: Create virtualized email list with filters, email detail view with sanitized HTML and attachments, inline reply/forward, keyboard shortcuts</name>
  <files>
    src/components/email/email-list.tsx
    src/components/email/email-list-row.tsx
    src/components/email/email-filters.tsx
...
- Render EmailActionsBar below header. Pass `onReplyAction` callback.
...
- **Inline reply area:** Below the email body, conditionally render `<EmailInlineReply>` when `replyMode` state is set. State: `replyMode: null | "reply" | "replyAll" | "forward"`. The `onReplyAction` callback from EmailActionsBar sets this state. `onClose` resets to null. This implements the locked decision: "Reply/forward: opens inline in detail pane (compose area below original email)".
...
- Reply/ReplyAll/Forward buttons call a callback prop `onReplyAction(mode: "reply" | "replyAll" | "forward")` to expand the inline reply area in email-detail.tsx (per locked decision: "Reply/forward: opens inline in detail pane, compose area below original email"). These do NOT open the compose popup — the compose popup is only for new email composition.

Create `src/components/email/email-inline-reply.tsx`:
- "use client" component that renders inline below the original email body in the detail pane. Per locked user decision: "Reply/forward: opens inline in detail pane (compose area below original email)".
- Props: `mode` ("reply" | "replyAll" | "forward"), `originalEmail` (full email data), `onClose` callback, `onSent` callback.
- Layout: expandable section below the email body with a border-top separator.
- Header: small "Antwort an {recipients}" or "Weiterleitung" label with close (X) button.
- Recipient fields: pre-filled based on mode. Reply: original sender in An. ReplyAll: original sender + all original To/CC in An/CC. Forward: empty An field.
- Compact recipient bar (tag-style chips) with option to expand CC/BCC.
- TipTap editor (reuse ComposeEditor from Plan 03 when available, or create a minimal inline version). Toolbar: Bold, Italic, Link, List, Attach file.
- Original email quoted below editor as blockquote with ">" styling. German date format header: "Am {date} schrieb {sender}:".
- Forward mode: show original attachments as pre-selected checkboxes (individually deselectable per user decision).
- Akte typeahead field (optional, for linking outgoing reply to a case).
- "Senden" button: POST /api/email-send. Shows 10-second undo toast. "Verwerfen" button: discard with confirmation if content edited.
- Signature auto-appended above quoted text (load via signature.ts from Plan 03).
- Validation: warn if Reply-All with 5+ recipients per user decision.
...
- Register shortcuts in the Command Palette (Cmd+K) via existing CommandPalette integration.
  ...
  <done>Virtualized email list renders with infinite scroll, sender/subject/date/preview/badges per row. Filter bar filters by status/akte/verantwortlicher/unread. Email detail shows sanitized HTML body with attachment download strip. Action bar has all 8 buttons. Reply/ReplyAll/Forward open inline compose area below original email body (not popup). Gmail-style keyboard shortcuts work for navigation and actions. Bulk actions work via checkboxes.</done>
</task>
