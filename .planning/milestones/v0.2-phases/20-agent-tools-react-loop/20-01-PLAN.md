...
<task type="auto">
  <name>Task 2: Create 12 read tool modules</name>
  <files>
    src/lib/helena/tools/_read/read-akte.ts
    src/lib/helena/tools/_read/read-akte-detail.ts
    src/lib/helena/tools/_read/read-dokumente.ts
  ...
  <action>
Create all 12 read tool modules following the plugin pattern from RESEARCH.md. Every tool file exports one `create*Tool(ctx: ToolContext)` function that returns `tool({ description, parameters, execute })`.

**Common patterns for ALL read tools:**
...
- Truncate large text fields in results to prevent token bloat. Content fields max 2000 chars, descriptions max 500 chars.
...
<task type="auto">
  <name>Task 3: Create 6 write tool modules</name>
  <files>
    src/lib/helena/tools/_write/create-draft-dokument.ts
    src/lib/helena/tools/_write/create-draft-frist.ts
    src/lib/helena/tools/_write/create-notiz.ts
  ...
  <action>
Create all 6 write tool modules following the same plugin pattern. Every tool file exports one `create*Tool(ctx: ToolContext)` function that returns `tool({ description, parameters, execute })`.

**Common patterns for ALL write tools:**
- Same conventions as read tools (English descriptions, Zod schemas, ToolResult shape, abort check)
...
- Exception: create-alert creates HelenaAlert directly (alerts ARE the output, not drafts)
...
6. **create-draft-zeiterfassung.ts** -- Parameters: `beschreibung` (string), `dauer` (number, minutes), optional `datum` (ISO date, default today), optional `akteId`. Creates HelenaDraft with typ=NOTIZ (no ZEITERFASSUNG in HelenaDraftTyp enum -- use NOTIZ with meta.subtype="zeiterfassung"). Returns: `{ draftId, typ, beschreibung, dauer, status }`.
  ...
  <done>
    ...
    - create-alert creates HelenaAlert directly
    - All files compile without TypeScript errors
  </done>
</task>
