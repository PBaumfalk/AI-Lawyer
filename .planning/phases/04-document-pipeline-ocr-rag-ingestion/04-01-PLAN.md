...
Output: Working OCR pipeline that processes uploaded documents automatically, with visible status badges, admin monitoring dashboard, floating upload panel, and Meilisearch full-text indexing of OCR'd text.
</objective>
...
<task type="auto">
  <name>Task 2: OCR Processor + Worker Registration + Upload Pipeline + Upload Panel + OCR Badge + Admin Dashboard + Tag Management</name>
  <files>
    src/lib/queue/processors/ocr.processor.ts
    src/lib/queue/processors/preview.processor.ts
    src/worker.ts
  ...
  <action>
    ...
    **8. Create OCR status badge component (src/components/dokumente/ocr-status-badge.tsx):**
    ...
    **9. Create floating upload panel (src/components/dokumente/upload-panel.tsx + src/components/providers/upload-provider.tsx):**

    **UploadProvider (src/components/providers/upload-provider.tsx):**
    ...
    **UploadPanel (src/components/dokumente/upload-panel.tsx):**
    ...
    **10. Create tag management API (src/app/api/dokumente/tags/route.ts):**
    ...
    **11. Create tag management settings page (src/app/(dashboard)/einstellungen/dokument-tags/page.tsx):**
  ...
  <verify>
    ...
    <manual>Check that upload API enqueues OCR jobs, OCR processor handles PDF/image/text routing, admin pipeline API returns queue stats, upload panel renders, tag management page works</manual>
  </verify>
  <done>
    ...
    - OCR status badge shows colored status on documents
    - Floating upload panel persists across navigation, shows per-file progress
    - Tag management CRUD API and admin settings page with predefined categories
  </done>
</task>
