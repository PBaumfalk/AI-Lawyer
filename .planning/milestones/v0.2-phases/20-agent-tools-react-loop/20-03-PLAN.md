...
must_haves:
  truths:
    ...
    - "runHelenaAgent() provides a single entry point combining tools, orchestrator, guard, classifier, and rate limiter"
    ...
    - "Rate limiter enforces admin-configurable per-user per-hour request limits using ioredis"
  artifacts:
    ...
    - path: "src/lib/helena/rate-limiter.ts"
      provides: "Per-user per-hour rate limiting via ioredis"
  ...
  key_links:
    ...
    - from: "src/lib/helena/index.ts"
      to: "src/lib/helena/rate-limiter.ts"
      via: "checkRateLimit before runAgent"
...
<objective>
Build the Ollama response guard, complexity classifier, rate limiter, and unified Helena agent entry point that combines all Plan 01 (tools) and Plan 02 (orchestrator) pieces into a single callable API.

Purpose: This plan closes the loop -- after it, any downstream phase can call `runHelenaAgent()` with a user message and get back a complete agent response with trace. The response guard ensures Ollama compatibility, the classifier enables dual-mode execution, and the rate limiter prevents abuse.

Output: Four modules completing the `src/lib/helena/` library, plus the public `index.ts` entry point.
</objective>
...
From ioredis (already installed):
```typescript
import Redis from "ioredis";
// Already used in the project for BullMQ queues
```
</interfaces>
</context>
...
<task type="auto">
  <name>Task 1: Ollama response guard, complexity classifier, and rate limiter</name>
  <files>
    src/lib/helena/response-guard.ts
    src/lib/helena/complexity-classifier.ts
    src/lib/helena/rate-limiter.ts
...
**src/lib/helena/rate-limiter.ts** -- Per-user per-hour rate limiting via ioredis:

Per CONTEXT.md locked decision: "Rate limiting: Admin-configurable rate limits per user per hour."

```typescript
export interface RateLimitResult {
  allowed: boolean;
  remaining: number;      // Requests remaining in the current window
  limit: number;          // Current limit for this user
  resetAt: Date;          // When the window resets
  message?: string;       // German error message if not allowed
}
...
Implementation:
- Read the rate limit from SystemSettings table: key `ai.helena.rate_limit_per_hour` (default: 60 requests/hour)
- Use ioredis to track per-user counters with a sliding window:
  - Redis key: `helena:ratelimit:${userId}`
  - Use `INCR` + `EXPIRE` pattern: increment counter, set 3600s TTL on first use
  - If counter exceeds limit, return `{ allowed: false, remaining: 0, limit, resetAt, message: "Du hast das Limit von {limit} Helena-Anfragen pro Stunde erreicht. Bitte warte bis {resetAt}." }`
- Import Redis client from existing project infrastructure (check for shared redis instance in `src/lib/redis.ts` or create connection using `REDIS_URL` env var)
- Graceful degradation: If Redis is unavailable (connection error), allow the request (fail open) and log a warning
- Export `checkRateLimit` and `RateLimitResult`
  </action>
  <verify>
  ...
  <done>
    ...
    - rate-limiter.ts exports checkRateLimit with ioredis-based per-user per-hour sliding window
    - Rate limit reads admin-configurable limit from SystemSettings (default 60/hour)
...
export interface HelenaAgentResult {
  ...
  rateLimited?: boolean;              // true if rate limit was hit
}
```

Implement `runHelenaAgent(options: HelenaAgentOptions): Promise<HelenaAgentResult>`:

1. **Rate limit check:** Call `checkRateLimit({ userId: options.userId, prisma: options.prisma })` BEFORE doing any other work. If `allowed === false`, return immediately with `{ text: result.message, rateLimited: true, ... }` (no agent run, no token usage). This enforces the locked decision "Rate limiting: Admin-configurable rate limits per user per hour."

2. **Get Helena user ID:** `const helenaUserId = await getHelenaUserId() ?? "system";`

3. **Classify complexity:** If `mode === "auto"` (default), call `classifyComplexity(options.message)` to determine mode and tier. If mode explicitly set to "inline" or "background", use that with tier from classifier.

4. **Get model for tier:** Call `getModelForTier(tier)` to get the LanguageModel instance. Also get model name for token budget.

5. **Create tools:** `const tools = createHelenaTools({ prisma, userId, userRole, akteId, helenaUserId })`

6. **Build system prompt:** `const systemPrompt = buildSystemPrompt({ tools, akteId, userName, helenaMemory })`

7. **Build messages:** Combine conversationHistory with new user message:
   ```typescript
...
8. **Determine provider:** `const providerName = await getProviderName()`. If provider is "ollama", pass `ollamaResponseGuard` as `repairToolCall`. For cloud providers, set to undefined.
...
10. **Handle inline cap reached:** If mode was "inline" and finishReason indicates the step cap was hit (not all steps used but tool calls still pending):
   - Set `capReached = true`
...
11. **Auto-escalation on stall:** If result.stalled AND tier < 3:
...
12. **Return HelenaAgentResult** with all fields populated.
...
```
  ...
  <done>
    ...
    - runHelenaAgent checks rate limit FIRST before any other work
    - Rate-limited requests return immediately with German message, no token usage
...
5. Rate limiter uses ioredis with per-user per-hour sliding window
6. index.ts wires all components: rate-limit -> tools -> orchestrator -> guard -> classifier
7. Auto-escalation caps at 1 retry to prevent infinite loops
</verification>

<success_criteria>
- runHelenaAgent() is callable with { prisma, userId, userRole, userName, akteId, message }
- Rate limit check runs before agent execution, returns German message when limit exceeded
